...
#'
#' This function calculates for each plot, height, species and year the number of trees per ha, and the number and percentage of subplots in which the species is regenerating. This calculation is designed for core areas, that consist of different subplots.
#'
#' @inheritParams calculate_regeneration
#'
#' @return dataframe with columns plot, species, year, height, min_number_of_trees_ha, max_number_of_trees_ha, number_of_subplots_with_regeneration and perc_subplots_with_regeneration
#'
#' @examples
#' \dontrun{
...
calculate_regeneration_core_area_height_species <- function(data_regeneration) {
  by_plot_species <- data_regeneration %>%
    ...
    group_by(
      .data$plot_id, .data$year, .data$period, .data$height_class, .data$species
    ) %>%
    summarise(
