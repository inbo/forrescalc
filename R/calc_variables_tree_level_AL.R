...
calc_variables_tree_level_AL <-
  function(data_dendro, data_stems, height_model) {
    data_dendro1 <- data_dendro %>%
      select(-.data$tree_number) %>%     # treenumber hier reeds verwijderen
      left_join(
        data_stems %>%
      ...
      mutate(
        individual = (.data$ind_sht_cop == 10 | .data$ind_sht_cop == 12)  # individual = TRUE voor elke boom of hakhoutstoof (om achteraf stamtal te berekenen)
      ) %>%
      left_join(
        height_model,
    ...
    data_dendro2 <- data_dendro1 %>%
      ...
      # !! calcheight wordt hier op niveau van trees berekend
      # zou ook nog eens op niveau van shoots moeten berekend worden
      mutate(
        calc_height_r =
          ifelse(
